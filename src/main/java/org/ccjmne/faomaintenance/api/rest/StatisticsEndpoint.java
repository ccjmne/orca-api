		...
		for (final Date date : dates) {
			...
			if (mostAccurate != null) {
				for (final Entry<String, List<String>> siteHistory : mostAccurate.getValue().entrySet()) {
					...
					siteHistory.getValue().forEach(registrationNumber -> stats.register(employeesStatistics.get(registrationNumber).get(date)));
		...
		for (final Date date : dates) {
			...
			if (mostAccurate != null) {
				for (final String registrationNumber : mostAccurate.getValue()) {
					stats.register(employeesStatistics.get(registrationNumber).get(date));
				}
			}
