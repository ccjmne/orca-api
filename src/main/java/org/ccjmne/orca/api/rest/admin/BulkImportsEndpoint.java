  ...
  public void bulkImportSites(final List<Map<String, String>> sites) {
    Transactions.with(this.ctx, transactionCtx -> {
      ...
      final Row3<String, Integer, String>[] tags = sites.stream().flatMap(s -> s.entrySet().stream()
          ...
          }).map(e -> DSL.row(s.get(SITES.SITE_EXTERNAL_ID.getName()), Integer.valueOf(e.getKey()), e.getValue()))).toArray(Row3[]::new);

      if (tags.length > 1) {
        final Table<Record3<String, Integer, String>> tagsTable = DSL.<String, Integer, String> values(tags)
            .asTable("unused", "site", "tag", "value");
        transactionCtx
