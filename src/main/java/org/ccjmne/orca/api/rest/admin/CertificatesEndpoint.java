	...
	public void udpateTtdf(final Integer trty_pk, final Map<String, Object> ttdf, final DSLContext transactionCtx) {
		final Integer ttdf_pk = Optional.ofNullable((Integer) ttdf.get(TRAININGTYPES_DEFS.TTDF_PK.getName()))
			.map(pk -> transactionCtx.update(TRAININGTYPES_DEFS)
				...
				.set(TRAININGTYPES_DEFS.TTDF_CERTIFIED,      DSL.val(            (Boolean) ttdf.get(TRAININGTYPES_DEFS.TTDF_CERTIFIED.getName())))
				...
				.columns(TRAININGTYPES_DEFS.TTDF_TRTY_FK, TRAININGTYPES_DEFS.TTDF_EFFECTIVE_FROM, TRAININGTYPES_DEFS.TTDF_PRESENCEONLY, TRAININGTYPES_DEFS.TTDF_EXTENDVALIDITY, TRAININGTYPES_DEFS.TTDF_CERTIFIED)
				.values(
					DSL.val(trty_pk),
					...
					DSL.val(            (Boolean) ttdf.get(TRAININGTYPES_DEFS.TTDF_CERTIFIED.getName()))
				)
				.returning(TRAININGTYPES_DEFS.TTDF_PK)
				.fetchOne().get(TRAININGTYPES_DEFS.TTDF_PK));
